      SUBROUTINE MYUFBINT(LUNIN,USR,I1,I2,IRET,STR)

C$$$  SUBPROGRAM DOCUMENTATION BLOCK
C
C SUBPROGRAM:    MYUFBINT
...
C   AT ALL. IF MYUFBINT IS READING VALUES, THEN EITHER BUFR ARCHIVE
...
C
C REMARKS:
C    1) MYUFBINT CAN ALSO BE CALLED TO PROVIDE INFORMATION ABOUT A SINGLE
...
C       FOR EXAMPLE:
C
C       CALL MYUFBINT(LUNIN,PLEVL,1, 50,IRET,'{PLEVL}')
C
...
C
C    2) WHEN THE BUFR FILE IS OPEN FOR OUTPUT, MYUFBINT CAN BE USED TO
...
C       ENDDO
C
C              ! The call to MYUFBINT here will not only store the 50
...
C       CALL MYUFBINT(LUNIN,DROBS,2, 50,IRET,'WSPD WDIR')
C
...
C         TO ZERO BY DEFUALT).  A CALL TO MYUFBINT IS FIRST NEEDED TO
...
C
C              ! This call to MYUFBINT will return the number of
C              !  replications ("levels") in "PLEVL" for subset
C              !  "REPT_IN"" !  {see 1) above in REMARKS}
C             CALL MYUFBINT(LUBFI,REPS_8,1,1,IRET,'{PLEVL}')
C             IREPS = REPS_8
C
C             IYR = OBSI(1)
C             IMO = OBSI(2)
...
C             PLEVL = GETBMISS()
C
C              ! The call to MYUFBINT here will allocate the space to
...
C             CALL MYUFBINT(LUBFJ,PLEVL,5,IREPS,IRET,
...
C  --------------------------------
      ...
      IF(I1.LE.0) THEN
         IF(IPRT.GE.0) THEN
      CALL ERRWRT('+++++++++++++++++++++WARNING+++++++++++++++++++++++')
      ERRSTR = 'BUFRLIB: MYUFBINT - 3rd ARG. (INPUT) IS .LE. 0, ' //
     .   'SO RETURN WITH 5th ARG. (IRET) = 0; 6th ARG. (STR) ='
      ...
      CALL ERRWRT('+++++++++++++++++++++WARNING+++++++++++++++++++++++')
      ERRSTR = 'BUFRLIB: MYUFBINT - 4th ARG. (INPUT) IS .LE. 0, ' //
     .   'SO RETURN WITH 5th ARG. (IRET) = 0; 6th ARG. (STR) ='
      CALL ERRWRT(ERRSTR)
      CALL ERRWRT(STR)
...
C  ---------------------------------------------------------------------
      ...
      IF(IRET.EQ.0)  THEN
         IF(IO.EQ.0) THEN
            IF(IPRT.GE.1)  THEN
      CALL ERRWRT('+++++++++++++++++++++WARNING+++++++++++++++++++++++')
      ERRSTR = 'BUFRLIB: MYUFBINT - NO SPECIFIED VALUES READ IN, ' //
     .   'SO RETURN WITH 5th ARG. (IRET) = 0; 6th ARG. (STR) ='
      CALL ERRWRT(ERRSTR)
      CALL ERRWRT(STR)
      CALL ERRWRT('+++++++++++++++++++++WARNING+++++++++++++++++++++++')
      ...
      CALL ERRWRT('+++++++++++++++++++++WARNING+++++++++++++++++++++++')
      ERRSTR = 'BUFRLIB: MYUFBINT-NO SPECIFIED VALUES WRITTEN OUT, ' //
     .   'SO RETURN WITH 5th ARG. (IRET) = 0; 6th ARG. (STR) ='
      CALL ERRWRT(ERRSTR)
      CALL ERRWRT(STR)
...
100   RETURN
900   CALL BORT('BUFRLIB: MYUFBINT - BUFR FILE IS CLOSED, IT MUST BE'//
     . ' OPEN')
901   CALL BORT('BUFRLIB: MYUFBINT - A MESSAGE MUST BE OPEN IN BUFR '//
     . 'FILE, NONE ARE')
902   CALL BORT('BUFRLIB: MYUFBINT - LOCATION OF INTERNAL TABLE FOR '//
     . 'BUFR FILE DOES NOT AGREE WITH EXPECTED LOCATION IN INTERNAL '//
     . 'SUBSET ARRAY')
903   WRITE(BORT_STR1,'("BUFRLIB: MYUFBINT-MNEMONIC STRING READ IN IS'//
     . ': ",A)') STR
