      SUBROUTINE MYUFBREP(LUNIO,USR,I1,I2,IRET,STR)

C$$$  SUBPROGRAM DOCUMENTATION BLOCK
C
C SUBPROGRAM:    MYUFBREP
...
C
C   THE DIFFERENCE IN THE WAY MYUFBREP WORKS AS COMPARED TO UFBINT IS IN
C   THE WAY THE MNEMONIC STRING IS INTERPRETED TO DEFINE WHICH ELEMENTS
C   ARE PROCESSED AND IN WHAT ORDER.  MYUFBREP INTERPRETS THE FIRST
...
C   {..,A,..,B,..,C,..,D,..,A,..,C,..,D,..,B,..
C         A,..,B,..,D,..,C,..,A,..,C,..,B,..,D,..},
C   THEN READING A SUBSET VIA MYUFBREP WITH STR = "A B C D" RETURNS THE
...
C         A3, B3, C3, D3,
C         A4, B4, C4, D4 )
C   NOTE THAT, WHEN USING MYUFBREP, THE ORDER OF THE NON-PIVOT MNEMONICS
C   BETWEEN EACH PIVOT IS IMMATERIAL, I.E., IN THE ABOVE EXAMPLE, MYUFBREP
...
C   IS SIMILAR TO MYUFBREP, EXCEPT THAT UFBSTP ALWAYS STEPS FORWARD WHEN
...
C                           MYUFBREP DOES NOT NEED THIS CHECK, AND IT
C                           INTERFERES WITH WHAT MYUFBREP CAN DO
C                           OTHERWISE
...
C  --------------------------------
      ...
      IF(I1.LE.0) THEN
         IF(IPRT.GE.0) THEN
      CALL ERRWRT('+++++++++++++++++++++WARNING+++++++++++++++++++++++')
      ERRSTR = 'BUFRLIB: MYUFBREP - 3rd ARG. (INPUT) IS .LE. 0, ' //
     .   'SO RETURN WITH 5th ARG. (IRET) = 0; 6th ARG. (STR) ='
      ...
      CALL ERRWRT('+++++++++++++++++++++WARNING+++++++++++++++++++++++')
      ERRSTR = 'BUFRLIB: MYUFBREP - 4th ARG. (INPUT) IS .LE. 0, ' //
     .   'SO RETURN WITH 5th ARG. (IRET) = 0; 6th ARG. (STR) ='
      CALL ERRWRT(ERRSTR)
      CALL ERRWRT(STR)
...
C  -------------------------------
      ...
      IF(IRET.EQ.0)  THEN
         IF(IO.EQ.0) THEN
            IF(IPRT.GE.1)  THEN
      CALL ERRWRT('+++++++++++++++++++++WARNING+++++++++++++++++++++++')
      ERRSTR = 'BUFRLIB: MYUFBREP - NO SPECIFIED VALUES READ IN, ' //
     .   'SO RETURN WITH 5th ARG. (IRET) = 0; 6th ARG. (STR) ='
      CALL ERRWRT(ERRSTR)
      CALL ERRWRT(STR)
      CALL ERRWRT('+++++++++++++++++++++WARNING+++++++++++++++++++++++')
      ...
      CALL ERRWRT('+++++++++++++++++++++WARNING+++++++++++++++++++++++')
      ERRSTR = 'BUFR: MYUFBREP - NO SPECIFIED VALUES WRITTEN OUT, ' //
     .   'SO RETURN WITH 5th ARG. (IRET) = 0; 6th ARG. (STR) ='
      CALL ERRWRT(ERRSTR)
      CALL ERRWRT(STR)
...
100   RETURN
900   CALL BORT('BUFRLIB: MYUFBREP - BUFR FILE IS CLOSED, IT MUST BE'//
     . ' OPEN')
901   CALL BORT('BUFRLIB: MYUFBREP - A MESSAGE MUST BE OPEN IN BUFR '//
     . 'FILE, NONE ARE')
902   CALL BORT('BUFRLIB: MYUFBREP - LOCATION OF INTERNAL TABLE FOR '//
     . 'BUFR FILE DOES NOT AGREE WITH EXPECTED LOCATION IN INTERNAL '//
     . 'SUBSET ARRAY')
903   WRITE(BORT_STR1,'("BUFR: MYUFBREP - MNEMONIC STRING READ IN IS'//
     . ': ",A)') STR
