      SUBROUTINE MYUPFTBV(LUNIT,NEMO,VAL,MXIB,IBIT,NIB)

C$$$  SUBPROGRAM DOCUMENTATION BLOCK
C
C SUBPROGRAM:    MYUPFTBV
...
C
C USAGE:    MYUPFTBV (LUNIT,NEMO,VAL,MXIB,IBIT,NIB)
...
Cf2py intent(out) IBIT
Cf2py depend(MXIB) IBIT
...
C     Figure out which bits are set.
      ...
      RETURN
900   CALL BORT('BUFRLIB: MYUPFTBV - INPUT BUFR FILE IS CLOSED, IT '//
     . 'MUST BE OPEN FOR INPUT')
901   WRITE(BORT_STR,'("BUFRLIB: MYUPFTBV - MNEMONIC ",A,'//
     . '" NOT FOUND IN TABLE B")') NEMO
      CALL BORT(BORT_STR)
902   WRITE(BORT_STR,'("BUFRLIB: MYUPFTBV - MNEMONIC ",A,'//
     . '" IS NOT A FLAG TABLE")') NEMO
      CALL BORT(BORT_STR)
903   CALL BORT('BUFRLIB: MYUPFTBV - IBIT ARRAY OVERFLOW')
      END
