      SUBROUTINE MYUFDUMPSTR(LUNIT,LUPRT,STROUT)

C$$$  SUBPROGRAM DOCUMENTATION BLOCK
C
C SUBPROGRAM:    MYUFDUMPSTR
...
C   THE INTERNAL SUBSET STRUCTURES.  EACH SUBROUTINE, UFBDMP AND MYUFDUMPSTR,
C   IS USEFUL FOR DIFFERENT DIAGNOSTIC PURPOSES, BUT IN GENERAL MYUFDUMPSTR
...
C
C USAGE:    CALL MYUFDUMPSTR (LUNIT, LUPRT)
...
C     STROUT   - CHARACTER*120: Return string of decoded BUFR subset
C
C
C REMARKS:
...
C  --------------------------------
      ...
      WRITE(STROUT(1),*)
      WRITE(STROUT(2),*) 'MESSAGE TYPE ',TAG(INODE(LUN))
      WRITE(STROUT(3),*)
...
C	   Print the number of replications
           ...
           FMT = '(11X,A,I6,1X,A)'
C           write(*,*) 'Here[0-0]: ', NV+3
           WRITE(STROUT(NV+3),FMT) NEMO2(1:LNM2), NUMREP(NSEQ), 
     .                             'REPLICATIONS'
C           write(*,*) 'Here[0-1]: ', STROUT(NV+3)
...
C           write(*,*) 'Here[1-0]: ', NV+3
               FMT = '(4X,A,2X,A,2X,A,I6,2X,A)'
               WRITE(STROUT(NV+3),FMT) '++++++', NEMO2(1:LNM2),
     .                 'REPLICATION #', IDXREP(II), '++++++'
C           write(*,*) 'Here[1-1]: ', STROUT(NV+3)
...
C           write(*,*) 'Here[2-0]: ', NV+3
            WRITE(STROUT(NV+3),FMT) NUMB,NEMO,PMISS,UNIT,DESC
C           write(*,*) 'Here[2-1]: ', STROUT(NV+3)
         ELSE
...
C              this value.
               ...
               ENDIF
            ENDIF         
C           write(*,*) 'Here[3-0]: ', NV+3
            WRITE(STROUT(NV+3),FMT) NUMB,NEMO,RVAL,UNIT,DESC
C           write(*,*) 'Here[3-1]: ', STROUT(NV+3)
         ENDIF
...
C           write(*,*) 'Here[4-0]: ', NV+3
            WRITE(STROUT(NV+3),FMT) NUMB,NEMO,LCHR,NCHR,UNIT,DESC
C           write(*,*) 'Here[4-1]: ', STROUT(NV+3)
         ELSE
            FMT = '(A6,2X,A10,2X,A,2X,"(",I3,")",A23,2X,A48)'
C           write(*,*) 'Here[5-0]: ', NV+3
            WRITE(STROUT(NV+3),FMT) NUMB,NEMO,LCHR2(1:NCHR),NCHR,
     .                              UNIT,DESC
C           write(*,*) 'Here[5-1]: ', STROUT(NV+3)
         ENDIF
      ENDIF

      ENDDO

C      write(*,*) 'Here[6-0]: ', NV+3
      WRITE(STROUT(NV+3),*) ' >>> END OF SUBSET <<< '
C      write(*,*) 'Here[6-1]: ', STROUT(NV+3)
C3     FORMAT(/' >>> END OF SUBSET <<< '/)

C  EXITS
C  -----

100   RETURN
900   CALL BORT('BUFR: MYUFDUMPSTR - INPUT BUFR FILE IS CLOSED, IT '//
     . 'MUST BE OPEN FOR INPUT')
901   CALL BORT('BUFR: MYUFDUMPSTR-INPUT BUFR FILE IS OPEN FOR '//
     . 'OUTPUT, IT MUST BE OPEN FOR INPUT')
902   CALL BORT('BUFR: MYUFDUMPSTR-A MESSAGE MUST BE OPEN IN INPUT '//
     . 'BUFR FILE, NONE ARE')
903   CALL BORT('BUFR: MYUFDUMPSTR - LOCATION OF INTERNAL TABLE FOR '//
     . 'INPUT BUFR FILE DOES NOT AGREE WITH EXPECTED LOCATION IN '//
     . 'INTERNAL SUBSET ARRAY')
904   CALL BORT('BUFRLIB: MYUFDUMPSTR - MXSEQ OVERFLOW')
905   CALL BORT('BUFRLIB: MYUFDUMPSTR - MXLS OVERFLOW')
      END
